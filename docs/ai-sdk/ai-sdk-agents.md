...
## How to Use Asset Generation Tools

This section demonstrates how to use the asset generation tools created for the BrandEntity application. These tools enable agents to generate, monitor, and manage graphic assets for brands and projects.

### Asset Generation Tools Overview

The asset generation system provides 5 specialized tools:

- **`generateAssets`** - Generate multiple graphic assets based on prompts
- **`getAssetTypes`** - List all available asset types and their configurations
- **`getGenerationStatus`** - Check the status of asset generation batches
- **`getAssetInfo`** - Get detailed information about specific assets
- **`listAssets`** - Browse and filter existing assets

### Setting Up Asset Generation Agent

First, import and configure the asset generation agent:

```ts
import { assetGenerationAgent } from '@/agents/asset-generation-agent';

// The agent is pre-configured with all asset generation tools
// and specialized system prompts for asset creation workflows
```

### Basic Asset Generation Workflow

Generate assets for a brand or project:

```ts
// Example: Generate a logo and icon for a coffee shop brand
const result = await assetGenerationAgent.generate({
  prompt: 'Create a modern logo and icon for a specialty coffee shop called "Bean Scene". Use warm brown and orange colors with clean, friendly design.',
});

// The agent will:
// 1. Understand the request for logo and icon assets
// 2. Call generateAssets tool with appropriate parameters
// 3. Return generation ID and status information
```

### Monitoring Generation Progress

Check the status of ongoing generations:

```ts
// Check status of a specific generation
const statusResult = await assetGenerationAgent.generate({
  prompt: 'Check the status of generation ID: abc-123-def-456',
});

// The agent will:
// 1. Parse the generation ID from the request
// 2. Call getGenerationStatus tool
// 3. Report progress, completion status, and success rate
```

### Asset Discovery and Management

Help users find and manage their assets:

```ts
// List recent assets for a project
const listResult = await assetGenerationAgent.generate({
  prompt: 'Show me all the completed assets for my ecommerce website project',
});

// The agent will:
// 1. Identify the request to list assets
// 2. Call listAssets tool with appropriate filters
// 3. Present assets with status, types, and URLs
```

### Advanced Asset Generation with Metadata

Generate assets with specific brand requirements:

```ts
// Generate assets with detailed brand specifications
const advancedResult = await assetGenerationAgent.generate({
  prompt: 'Create background images for our tech startup. Use our brand colors: #0066cc and #ffffff. Make them modern and professional with subtle gradients. Generate 3 different variations.',
});

// The agent will:
// 1. Parse brand colors and style requirements
// 2. Call generateAssets with metadata including colors and style
// 3. Generate multiple variations as requested
```

### Complete Agent Workflow Example

Here's a comprehensive example showing how an agent handles a complex asset generation request:

```ts
// User request: "I need assets for my new restaurant branding"
const workflowResult = await assetGenerationAgent.generate({
  prompt: 'Create a complete brand asset package for "Bella Vista Bistro", an Italian restaurant. Include logo, background images, and menu icons. Use warm colors like terracotta, cream, and olive green. Make it elegant and welcoming.',
});

// Agent workflow:
// 1. Analyzes the comprehensive branding request
// 2. Calls getAssetTypes to understand available options
// 3. Calls generateAssets multiple times for different asset types
// 4. Monitors progress with getGenerationStatus
// 5. Provides summary of all generated assets
// 6. Offers suggestions for additional assets or refinements
```
...
```ts
// If generation fails, the agent provides helpful feedback
const errorResult = await assetGenerationAgent.generate({
  prompt: 'Check why my logo generation failed',
});

// Agent will:
// 1. Call getGenerationStatus or getAssetInfo
// 2. Analyze error messages
// 3. Suggest solutions or alternative approaches
```

### Integration with Existing Systems

The asset generation tools integrate seamlessly with your existing BrandEntity infrastructure:

- **Database Integration**: Assets are stored in your Prisma database
- **Project Management**: Assets are linked to projects and brands
- **File Storage**: Generated images are stored and served via your API
- **Real-time Updates**: Status changes trigger appropriate UI updates

### Best Practices for Asset Generation

When using the asset generation agent:

1. **Be Specific**: Provide detailed descriptions including colors, style, and purpose
2. **Use Brand Context**: Reference existing brand colors and assets for consistency
3. **Monitor Progress**: Check generation status for long-running batches
4. **Iterate**: Use feedback to refine and improve generated assets
5. **Batch Wisely**: Group related assets in single generations for efficiency

### Tool Capabilities Summary

| Tool | Purpose | Key Features |
|------|---------|--------------|
| `generateAssets` | Create new assets | Multiple types, brand context, metadata support |
| `getAssetTypes` | Discover options | Complete type catalog with descriptions |
| `getGenerationStatus` | Monitor progress | Real-time status, success rates, completion tracking |
| `getAssetInfo` | Asset details | URLs, metadata, generation history |
| `listAssets` | Browse assets | Filtering, pagination, status-based queries |

This asset generation system enables agents to provide a complete creative workflow, from initial concept to final asset delivery, all while maintaining brand consistency and project organization.
